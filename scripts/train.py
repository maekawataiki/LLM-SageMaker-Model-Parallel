...
@smp.step
def train_step(model, input_ids, attention_mask, labels, args):
    """Train step."""
...
# smdistributed: Define smp.step. Return any tensors needed outside.
@smp.step
def test_step(model, input_ids, attention_mask, labels):
    """Test step."""
    loss = model(input_ids=input_ids, attention_mask=attention_mask, labels=labels)["loss"]
    return loss
    ...
    with torch.no_grad():
        for batch_idx, input_data in enumerate(dataloader):
            if use_bert_data:
                ...
                input_ids, attention_mask, labels = input_data
            if batch_idx >= num_batches:
                break

            loss += test_step(model, input_ids, attention_mask, labels).reduce_mean()
            n_batches += 1
...
def train(  # pylint: disable=too-many-arguments,too-many-branches,too-many-locals,too-many-statements
    ...
    for index in range(start_train_path_index, args.epochs * len(train_paths)):
        logging.info("Starting epoch %s.", index)
        ...
        for batch_idx, input_data in enumerate(train_dataloader):
            ...
            if args.logits_output:
                train_output = train_step(model, input_ids, attention_mask, labels, args)
                ...
                loss_mb = train_step(model, input_ids, attention_mask, labels, args)
...
def parse_args():  # pylint: disable=too-many-statements
    ...
    model_grp.add_argument("--tokenizer_name", type=str, default="", help="HF tokenizer name")
...
def main():  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    if args.fine_tune > 0 and args.delayed_param > 0 and smp.rank() == 0:
        ...
        print('model_state_dict saved to ', path)
    smp.barrier()
    # About zero_init:
    ...
    with smp.model_creation(
        ...
        if args.fine_tune > 0 and args.delayed_param == 0:
            ...
            if smp.rank() == 0:
                print(model)
            model.gpt_neox.config.hidden_act = 'gelu_new'
        else:
